...
export default class EpisodeSingle extends Component {
  ...
  changeExercises() {
    const exercises = Object.entries(this.props.navigation.state.params.exercises).map(([key, value], i) => {
  ...
  onDownload() {
    console.log(this.props.navigation.state.params.tracks[0].audioUrl);
    RNFS.downloadFile({fromUrl:this.props.navigation.state.params.tracks[0].audioUrl, toFile: 'cache'}).promise.then(res => {
      console.log('done');
      console.log(res);
    });
  }
